...
namespace agg
{
    ...
    struct comp_op_rgba_src_in : blender_base<ColorT, Order>
    {
        ...
        static AGG_INLINE void blend_pix(value_type* p, 
            value_type r, value_type g, value_type b, value_type a, cover_type cover)
        {
            float64 da = ColorT::to_float64(p[Order::A]);
            if (da > 0)
            {
                rgba s = get(r, g, b, a, cover);
                rgba d = get(p, cover_full - cover);
    ...
    struct comp_op_rgba_dst_in : blender_base<ColorT, Order>
    {
        ...
        {
            float64 sa = ColorT::to_float64(a);
            rgba d = get(p, cover_full - cover);
            rgba d2 = get(p, cover);
            d.r += d2.r * sa;
            d.g += d2.g * sa;
    ...
    struct comp_op_rgba_src_out : blender_base<ColorT, Order>
    {
            ...
            value_type r, value_type g, value_type b, value_type a, cover_type cover)
        {
            rgba s = get(r, g, b, a, cover);
            rgba d = get(p, cover_full - cover);
            float64 d1a = 1 - ColorT::to_float64(p[Order::A]);
            d.r += s.r * d1a;
            d.g += s.g * d1a;
            d.b += s.b * d1a;
            d.a += s.a * d1a;
    ...
    struct comp_op_rgba_dst_out : blender_base<ColorT, Order>
    {
            ...
            value_type, value_type, value_type, value_type a, cover_type cover)
        {
            rgba d = get(p, cover_full - cover);
            rgba dc = get(p, cover);
            float64 s1a = 1 - ColorT::to_float64(a);
            d.r += dc.r * s1a;
            d.g += dc.g * s1a;
            d.b += dc.b * s1a;
            d.a += dc.a * s1a;
    ...
    struct comp_op_rgba_plus : blender_base<ColorT, Order>
    {
            ...
            value_type r, value_type g, value_type b, value_type a, cover_type cover)
        {
            ...
            {
                rgba d = get(p);
                d.a = sd_min(d.a + s.a, (float64)1.0);
    ...
    struct comp_op_rgba_minus : blender_base<ColorT, Order>
    {
            ...
            value_type r, value_type g, value_type b, value_type a, cover_type cover)
        {
            ...
            if (s.a > 0)
            {
                rgba d = get(p);
                d.a += s.a - s.a * d.a;
                d.r = sd_max(d.r - s.r, (float64)0.0);
                d.g = sd_max(d.g - s.g, (float64)0.0);
                d.b = sd_max(d.b - s.b, (float64)0.0);
                set(p, clip(d));
            }
        }
    };
    ...
    struct comp_op_rgba_overlay : blender_base<ColorT, Order>
    {
        ...
        // 
        // Da' = Sa + Da - Sa.Da
        static AGG_INLINE float64 calc(float64 dca, float64 sca, float64 da, float64 sa, float64 sada, float64 d1a, float64 s1a)
        {
            return (2 * dca <= da) ? 
    ...
    struct comp_op_rgba_color_dodge : blender_base<ColorT, Order>
    {
        ...
        static AGG_INLINE float64 calc(float64 dca, float64 sca, float64 da, float64 sa, float64 sada, float64 d1a, float64 s1a)
        {
            if (sca < sa) return sada * sd_min((float64)1.0, (dca / da) * sa / (sa - sca)) + sca * d1a + dca * s1a;
    ...
    struct comp_op_rgba_color_burn : blender_base<ColorT, Order>
    {
        ...
        static AGG_INLINE float64 calc(float64 dca, float64 sca, float64 da, float64 sa, float64 sada, float64 d1a, float64 s1a)
        {
            if (sca > 0) return sada * (1 - sd_min((float64)1.0, (1 - dca / da) * sa / sca)) + sca * d1a + dca * s1a;
