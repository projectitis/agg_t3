...
namespace agg
{
    ...
    const float64 bezier_arc_angle_epsilon = 0.01;
    ...
    void arc_to_bezier(float64 cx, float64 cy, float64 rx, float64 ry, 
                       ...
                       float64* curve)
    {
        float64 x0 = cos(sweep_angle / 2.0);
        float64 y0 = sin(sweep_angle / 2.0);
        float64 tx = (1.0 - x0) * 4.0 / 3.0;
        float64 ty = y0 - tx * x0 / y0;
        float64 px[4];
        float64 py[4];
        px[0] =  x0;
        py[0] = -y0;
        ...
        float64 sn = sin(start_angle + sweep_angle / 2.0);
        float64 cs = cos(start_angle + sweep_angle / 2.0);
    ...
    //------------------------------------------------------------------------
    void bezier_arc::init(float64 x,  float64 y, 
                          ...
                          float64 sweep_angle)
    {
        ...
        float64 total_sweep = 0.0;
        float64 local_sweep = 0.0;
        float64 prev_sweep;
        m_num_vertices = 2;
    ...
    //--------------------------------------------------------------------
    void bezier_arc_svg::init(float64 x0, float64 y0, 
                              float64 rx, float64 ry, 
                              float64 angle,
                              bool large_arc_flag,
                              bool sweep_flag,
                              float64 x2, float64 y2)
    {
        ...
        //------------------------
        float64 dx2 = (x0 - x2) / 2.0;
        float64 dy2 = (y0 - y2) / 2.0;

        float64 cos_a = cos(angle);
        float64 sin_a = sin(angle);

        // Calculate (x1, y1)
        //------------------------
        float64 x1 =  cos_a * dx2 + sin_a * dy2;
        float64 y1 = -sin_a * dx2 + cos_a * dy2;
        ...
        //------------------------
        float64 radii_check = px1/prx + py1/pry;
        if(radii_check > 1.0) 
        {
        ...
        // Calculate (cx1, cy1)
        //------------------------
        float64 sign = (large_arc_flag == sweep_flag) ? -1.0 : 1.0;
        float64 sq   = (prx*pry - prx*py1 - pry*px1) / (prx*py1 + pry*px1);
        float64 coef = sign * sqrt((sq < 0) ? 0 : sq);
        float64 cx1  = coef *  ((rx * y1) / ry);
        float64 cy1  = coef * -((ry * x1) / rx);

        //
        // Calculate (cx, cy) from (cx1, cy1)
        //------------------------
        float64 sx2 = (x0 + x2) / 2.0;
        float64 sy2 = (y0 + y2) / 2.0;
        float64 cx = sx2 + (cos_a * cx1 - sin_a * cy1);
        float64 cy = sy2 + (sin_a * cx1 + cos_a * cy1);

        // Calculate the start_angle (angle1) and the sweep_angle (dangle)
        //------------------------
        float64 ux =  (x1 - cx1) / rx;
        float64 uy =  (y1 - cy1) / ry;
        float64 vx = (-x1 - cx1) / rx;
        float64 vy = (-y1 - cy1) / ry;
        float64 p, n;
        ...
        if(v < -1.0) v = -1.0;
        if(v >  1.0) v =  1.0;
        float64 start_angle = sign * acos(v);
        ...
        if(v < -1.0) v = -1.0;
        if(v >  1.0) v =  1.0;
        float64 sweep_angle = sign * acos(v);
