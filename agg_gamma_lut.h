...
namespace agg
{
        ...
        void gamma(float64 g) 
        {
            ...
            for(i = 0; i < gamma_size; i++)
            {
                m_dir_gamma[i] = (HiResT)
                    uround(pow(i / float64(gamma_mask), m_gamma) * float64(hi_res_mask));
            }
            ...
            for(i = 0; i < hi_res_size; i++)
            {
                m_inv_gamma[i] = (LoResT)
                    uround(pow(i / float64(hi_res_mask), inv_g) * float64(gamma_mask));
            }
        }
	...
	*/
    ...
    public:
        ...
        {
			static const float64 y = 1 / 255.0;
            return float(x * y);
        }
