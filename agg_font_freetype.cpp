...
namespace agg
{
    ...
    //------------------------------------------------------------------------
    static inline int dbl_to_plain_fx(float64 d)
    {
        return int(d * 65536.0);
    }

    //------------------------------------------------------------------------
    static inline float64 int26p6_to_dbl(int p)
    {
        return float64(p) / 64.0;
    }

    //------------------------------------------------------------------------
    static inline int dbl_to_int26p6(float64 p)
    {
        ...
        m_rasterizer()
    {
        ...
        m_last_error = 0; //FT_Init_FreeType(&m_library);
    ...
    //------------------------------------------------------------------------
    float64 font_engine_freetype_base::ascender() const
    {
        if(m_cur_face)
        {
    ...
    //------------------------------------------------------------------------
    float64 font_engine_freetype_base::descender() const
    {
        if(m_cur_face)
        {
    ...
    //------------------------------------------------------------------------
    bool font_engine_freetype_base::height(float64 h)
    {
        m_height = int(h * 64.0);
        if(m_cur_face)
        {
    ...
    //------------------------------------------------------------------------
    bool font_engine_freetype_base::width(float64 w)
    {
        m_width = int(w * 64.0);
        if(m_cur_face)
        {
...
//                                     m_hinting ? FT_LOAD_FORCE_AUTOHINT : FT_LOAD_NO_HINTING);
                                           ...
                                           float64* x, float64* y)
    {
        if(m_cur_face && first && second && FT_HAS_KERNING(m_cur_face))
        {
            ...
            float64 dx = int26p6_to_dbl(delta.x);
            float64 dy = int26p6_to_dbl(delta.y);
            if(m_glyph_rendering == glyph_ren_outline ||
               m_glyph_rendering == glyph_ren_agg_mono ||
               m_glyph_rendering == glyph_ren_agg_gray8)
            {
