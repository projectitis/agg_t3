...
namespace agg
{
    ...
    public:
        ...
        line_image_scale(const Source& src, float64 height) :
            m_source(src), 
            m_height(height),
        ...
        float64 width()  const { return m_source.width(); }
        float64 height() const { return m_height; }
        ...
        { 
            if (m_scale < 1.0)
            {
                ...
                float64 src_y = (y + 0.5) * m_scale - 0.5;
            ...
            else
            {
                ...
                float64 src_y1 = (y + 0.5) * m_scale - 0.5;
                float64 src_y2 = src_y1 + m_scale;
                int h  = m_source.height() - 1;
    ...
    public:
        ...
        float64 width()      const { return m_height; }
    ...
    public:
        ...
        distance_interpolator4(int x1,  int y1, int x2, int y2,
                               ...
                               int len, float64 scale, int x, int y) :
            m_dx(x2 - x1),
            m_dy(y2 - y1),
            ...
            m_dist(iround(float64(x + line_subpixel_scale/2 - x2) * float64(m_dy) - 
                          float64(y + line_subpixel_scale/2 - y2) * float64(m_dx))),
    ...
    public:
        ...
        //---------------------------------------------------------------------
        void   scale_x(float64 s) { m_scale_x = s; }
        float64 scale_x() const   { return m_scale_x; }

        //---------------------------------------------------------------------
        void   start_x(float64 s) { m_start = iround(s * line_subpixel_scale); }
        float64 start_x() const   { return float64(m_start) / line_subpixel_scale; }

        //---------------------------------------------------------------------
        int subpixel_width() const { return m_pattern->line_width(); }
        int pattern_width() const { return m_pattern->pattern_width(); }
        float64 width() const { return float64(subpixel_width()) / line_subpixel_scale; }
