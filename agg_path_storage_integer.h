...
namespace agg
{
    ...
    template<class T, unsigned CoordShift=6> struct vertex_integer
    {
        ...
        unsigned vertex(float64* x_, float64* y_, 
                        float64 dx=0, float64 dy=0,
                        float64 scale=1.0) const
        {
            *x_ = dx + (float64(x >> 1) / coord_scale) * scale;
            *y_ = dy + (float64(y >> 1) / coord_scale) * scale;
            switch(((y & 1) << 1) | (x & 1))
            {
    ...
    public:
        ...
        {
            rect_d bounds( (float64)AGG_DBL_MAX, (float64)AGG_DBL_MAX, (float64)-AGG_DBL_MAX, (float64)-AGG_DBL_MAX);
            if(m_storage.size() == 0)
            {
    ...
    public:
        ...
        void init(const int8u* data, unsigned size, 
                  float64 dx, float64 dy, float64 scale=1.0)
        {
            m_data     = data;
            m_end      = data + size;
            m_ptr      = data;
