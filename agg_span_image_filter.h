...
namespace agg
{
    ...
    public:
        ...
        float64 filter_dx_dbl()            const { return m_dx_dbl; }
        float64 filter_dy_dbl()            const { return m_dy_dbl; }

        //--------------------------------------------------------------------
        void interpolator(interpolator_type& v)  { m_interpolator = &v; }
        void filter(image_filter_lut& v)         { m_filter = &v; }
        void filter_offset(float64 dx, float64 dy)
        {
            m_dx_dbl = dx;
            m_dy_dbl = dy;
        ...
        void filter_offset(float64 d) { filter_offset(d, d); }
    ...
    public:
        ...
        //--------------------------------------------------------------------
        float64 blur_x() const { return m_blur_x; }
        float64 blur_y() const { return m_blur_y; }
        void blur_x(float64 v) { m_blur_x = v; }
        void blur_y(float64 v) { m_blur_y = v; }
        void blur(float64 v) { m_blur_x = m_blur_y = v; }
        ...
        void prepare() 
        {
			...
			float64 scale_xy = scale_x * scale_y;
            ...
            m_rx     = uround(    scale_x * float64(image_subpixel_scale));
            m_rx_inv = uround(1.0/scale_x * float64(image_subpixel_scale));

            m_ry     = uround(    scale_y * float64(image_subpixel_scale));
            m_ry_inv = uround(1.0/scale_y * float64(image_subpixel_scale));
        }
    ...
    public:
        ...
        //--------------------------------------------------------------------
        float64 blur_x() const { return float64(m_blur_x) / float64(image_subpixel_scale); }
        float64 blur_y() const { return float64(m_blur_y) / float64(image_subpixel_scale); }
        void blur_x(float64 v) { m_blur_x = uround(v * float64(image_subpixel_scale)); }
        void blur_y(float64 v) { m_blur_y = uround(v * float64(image_subpixel_scale)); }
        void blur(float64 v)   { m_blur_x = 
                                m_blur_y = uround(v * float64(image_subpixel_scale)); }
