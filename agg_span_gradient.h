...
namespace agg
{
    ...
    public:
        ...
        float64 d1() const { return float64(m_d1) / gradient_subpixel_scale; }
        float64 d2() const { return float64(m_d2) / gradient_subpixel_scale; }

        //--------------------------------------------------------------------
        void interpolator(interpolator_type& i) { m_interpolator = &i; }
        void gradient_function(GradientF& gf) { m_gradient_function = &gf; }
        void color_function(ColorF& cf) { m_color_function = &cf; }
        void d1(float64 v) { m_d1 = iround(v * gradient_subpixel_scale); }
        void d2(float64 v) { m_d2 = iround(v * gradient_subpixel_scale); }
    ...
    struct gradient_linear_color
    {
        ...
        {
			// VFALCO 4/28/09 
            //return m_c1.gradient(m_c2, float64(v) / float64(m_size - 1));
            return m_c1.gradient(m_c2, float64(v) * m_mult );
			// VFALCO
        }
    ...
    public:
        static AGG_INLINE int calculate(int x, int y, int)
        {
            return uround(sqrt(float64(x)*float64(x) + float64(y)*float64(y)));
        }
    };
    ...
    public:
        ...
        //---------------------------------------------------------------------
        gradient_radial_focus(float64 r, float64 fx, float64 fy) : 
        ...
        //---------------------------------------------------------------------
        void init(float64 r, float64 fx, float64 fy)
        {
            m_r  = iround(r  * gradient_subpixel_scale);
            m_fx = iround(fx * gradient_subpixel_scale);
            m_fy = iround(fy * gradient_subpixel_scale);
        ...
        //---------------------------------------------------------------------
        float64 radius()  const { return float64(m_r)  / gradient_subpixel_scale; }
        float64 focus_x() const { return float64(m_fx) / gradient_subpixel_scale; }
        float64 focus_y() const { return float64(m_fy) / gradient_subpixel_scale; }
        ...
        int calculate(int x, int y, int) const
        {
            float64 dx = x - m_fx;
            float64 dy = y - m_fy;
            float64 d2 = dx * m_fy - dy * m_fx;
            float64 d3 = m_r2 * (dx * dx + dy * dy) - d2 * d2;
    ...
    private:
        ...
        void update_values()
        {
            ...
            //-------------------------
            m_r2  = float64(m_r)  * float64(m_r);
            m_fx2 = float64(m_fx) * float64(m_fx);
            m_fy2 = float64(m_fy) * float64(m_fy);
            float64 d = (m_r2 - (m_fx2 + m_fy2));
            if(d == 0)
            {
    ...
    public:
        static AGG_INLINE int calculate(int x, int y, int d) 
        { 
            return uround(fabs(atan2(float64(y), float64(x))) * float64(d) / pi);
        }
    };
