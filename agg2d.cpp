...
//
// 2018-04-24 Peter "Projectitis" Vullings (peter@projectitis.com)
//	 Changed agg::float64s to agg::float64s for microprocessor support
//
//----------------------------------------------------------------------------

#include "agg2d.h"

static const agg::float64 g_approxScale = 2.0;
...
//------------------------------------------------------------------------
void Agg2D::clipBox(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2)
{
    m_clipBox = RectF(x1, y1, x2, y2);
    int rx1 = int(x1);
    int ry1 = int(y1);
    int rx2 = int(x2);
    int ry2 = int(y2);
...
//------------------------------------------------------------------------
void Agg2D::masterAlpha(agg::float64 a)
{
    m_masterAlpha = a;
...
//------------------------------------------------------------------------
agg::float64 Agg2D::masterAlpha() const
{
    return m_masterAlpha;
}

//------------------------------------------------------------------------
void Agg2D::antiAliasGamma(agg::float64 g)
{
    m_antiAliasGamma = g;
    updateRasterizerGamma();
}
...
//------------------------------------------------------------------------
agg::float64 Agg2D::antiAliasGamma() const
{
    return m_antiAliasGamma;
}

//------------------------------------------------------------------------
Agg2D::RectF Agg2D::clipBox() const
{
    return m_clipBox;
}
...
//------------------------------------------------------------------------
void Agg2D::worldToScreen(agg::float64& x, agg::float64& y) const
{
    m_transform.transform(&x, &y);
}

//------------------------------------------------------------------------
void Agg2D::screenToWorld(agg::float64& x, agg::float64& y) const
{
...
//------------------------------------------------------------------------
agg::float64 Agg2D::worldToScreen(agg::float64 scalar) const
{
    agg::float64 x1 = 0;
    agg::float64 y1 = 0;
    agg::float64 x2 = scalar;
    agg::float64 y2 = scalar;
    worldToScreen(x1, y1);
    worldToScreen(x2, y2);
...
//------------------------------------------------------------------------
agg::float64 Agg2D::screenToWorld(agg::float64 scalar) const
{
    agg::float64 x1 = 0;
    agg::float64 y1 = 0;
    agg::float64 x2 = scalar;
...
//------------------------------------------------------------------------
void Agg2D::alignPoint(agg::float64& x, agg::float64& y) const
{
    worldToScreen(x, y);
    x = floor(x) + 0.5;
    y = floor(y) + 0.5;
...
//------------------------------------------------------------------------
bool Agg2D::inBox(agg::float64 worldX, agg::float64 worldY) const
{
...
//------------------------------------------------------------------------
void Agg2D::rotate(agg::float64 angle)          { m_transform *= agg::trans_affine_rotation(angle);   }
void Agg2D::skew(agg::float64 sx, agg::float64 sy)    { m_transform *= agg::trans_affine_skewing(sx, sy);   }
void Agg2D::translate(agg::float64 x, agg::float64 y) { m_transform *= agg::trans_affine_translation(x, y); }
...
//------------------------------------------------------------------------
void Agg2D::scale(agg::float64 sx, agg::float64 sy)
{
...
//------------------------------------------------------------------------
void Agg2D::parallelogram(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2, const agg::float64* para)
{
...
//------------------------------------------------------------------------
void Agg2D::viewport(agg::float64 worldX1,  agg::float64 worldY1,  agg::float64 worldX2,  agg::float64 worldY2,
...
//------------------------------------------------------------------------
void Agg2D::fillLinearGradient(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2, Color c1, Color c2, agg::float64 profile)
{
    int i;
    ...
    agg::float64 k = 1.0 / agg::float64(endGradient - startGradient);
    ...
    for (; i < endGradient; i++)
    {
        m_fillGradient[i] = c1.gradient(c2, agg::float64(i - startGradient) * k);
    }
    for (; i < 256; i++)
    {
        m_fillGradient[i] = c2;
    }
    agg::float64 angle = atan2(y2-y1, x2-x1);
...
//------------------------------------------------------------------------
void Agg2D::lineLinearGradient(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2, Color c1, Color c2, agg::float64 profile)
{
    int i;
    ...
    for (; i < endGradient; i++)
    {
        m_lineGradient[i] = c1.gradient(c2, agg::float64(i - startGradient) * k);
    }
    for (; i < 256; i++)
    {
        m_lineGradient[i] = c2;
...
//------------------------------------------------------------------------
void Agg2D::fillRadialGradient(agg::float64 x, agg::float64 y, agg::float64 r, Color c1, Color c2, agg::float64 profile)
{
    int i;
    int startGradient = 128 - int(profile * 127.0);
    int endGradient   = 128 + int(profile * 127.0);
...
//------------------------------------------------------------------------
void Agg2D::lineRadialGradient(agg::float64 x, agg::float64 y, agg::float64 r, Color c1, Color c2, agg::float64 profile)
{
    int i;
    int startGradient = 128 - int(profile * 128.0);
    int endGradient   = 128 + int(profile * 128.0);
...
//------------------------------------------------------------------------
void Agg2D::fillRadialGradient(agg::float64 x, agg::float64 y, agg::float64 r, Color c1, Color c2, Color c3)
{
    int i;
    for (i = 0; i < 128; i++)
    {
        m_fillGradient[i] = c1.gradient(c2, agg::float64(i) / 127.0);
    }
    for (; i < 256; i++)
    {
        m_fillGradient[i] = c2.gradient(c3, agg::float64(i - 128) / 127.0);
    }
    m_fillGradientD2 = worldToScreen(r);
    worldToScreen(x, y);
    m_fillGradientMatrix.reset();
...
//------------------------------------------------------------------------
void Agg2D::lineRadialGradient(agg::float64 x, agg::float64 y, agg::float64 r, Color c1, Color c2, Color c3)
{
    int i;
    for (i = 0; i < 128; i++)
    {
        m_lineGradient[i] = c1.gradient(c2, agg::float64(i) / 127.0);
    }
    for (; i < 256; i++)
    {
        m_lineGradient[i] = c2.gradient(c3, agg::float64(i - 128) / 127.0);
    }
    m_lineGradientD2 = worldToScreen(r);
    worldToScreen(x, y);
    m_lineGradientMatrix.reset();
...
void Agg2D::fillRadialGradient(agg::float64 x, agg::float64 y, agg::float64 r)
{
    m_fillGradientD2 = worldToScreen(r);
    worldToScreen(x, y);
    m_fillGradientMatrix.reset();
...
//------------------------------------------------------------------------
void Agg2D::lineRadialGradient(agg::float64 x, agg::float64 y, agg::float64 r)
{
    m_lineGradientD2 = worldToScreen(r);
    worldToScreen(x, y);
    m_lineGradientMatrix.reset();
...
//------------------------------------------------------------------------
void Agg2D::lineWidth(agg::float64 w)
{
    m_lineWidth = w;
...
//------------------------------------------------------------------------
agg::float64 Agg2D::lineWidth() const
{
    return m_lineWidth;
}
...
//------------------------------------------------------------------------
void Agg2D::addLine(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2)
{
...
//------------------------------------------------------------------------
void Agg2D::line(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2)
{
    m_path.remove_all();
    addLine(x1, y1, x2, y2);
    drawPath(StrokeOnly);
...
//------------------------------------------------------------------------
void Agg2D::triangle(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2, agg::float64 x3, agg::float64 y3)
{
    m_path.remove_all();
    m_path.move_to(x1, y1);
    m_path.line_to(x2, y2);
...
//------------------------------------------------------------------------
void Agg2D::rectangle(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2)
{
    m_path.remove_all();
    m_path.move_to(x1, y1);
...
//------------------------------------------------------------------------
void Agg2D::roundedRect(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2, agg::float64 r)
{
    m_path.remove_all();
...
//------------------------------------------------------------------------
void Agg2D::roundedRect(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2, agg::float64 rx, agg::float64 ry)
{
    m_path.remove_all();
    agg::rounded_rect rc;
    rc.rect(x1, y1, x2, y2);
...
//------------------------------------------------------------------------
void Agg2D::roundedRect(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2,
                        agg::float64 rx_bottom, agg::float64 ry_bottom,
                        agg::float64 rx_top,    agg::float64 ry_top)
{
    m_path.remove_all();
    agg::rounded_rect rc;
    rc.rect(x1, y1, x2, y2);
...
//------------------------------------------------------------------------
void Agg2D::ellipse(agg::float64 cx, agg::float64 cy, agg::float64 rx, agg::float64 ry)
{
    m_path.remove_all();
...
//------------------------------------------------------------------------
void Agg2D::arc(agg::float64 cx, agg::float64 cy, agg::float64 rx, agg::float64 ry, agg::float64 start, agg::float64 sweep)
{
    m_path.remove_all();
...
//------------------------------------------------------------------------
void Agg2D::star(agg::float64 cx, agg::float64 cy, agg::float64 r1, agg::float64 r2, agg::float64 startAngle, int numRays)
{
    m_path.remove_all();
    agg::float64 da = agg::pi / agg::float64(numRays);
    agg::float64 a = startAngle;
    int i;
    for (i = 0; i < numRays; i++)
    {
        agg::float64 x = cos(a) * r2 + cx;
        agg::float64 y = sin(a) * r2 + cy;
...
//------------------------------------------------------------------------
void Agg2D::curve(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2, agg::float64 x3, agg::float64 y3)
{
    m_path.remove_all();
    m_path.move_to(x1, y1);
...
//------------------------------------------------------------------------
void Agg2D::curve(agg::float64 x1, agg::float64 y1, agg::float64 x2, agg::float64 y2, agg::float64 x3, agg::float64 y3, agg::float64 x4, agg::float64 y4)
{
    m_path.remove_all();
    m_path.move_to(x1, y1);
...
//------------------------------------------------------------------------
void Agg2D::polygon(agg::float64* xy, int numPoints)
{
    m_path.remove_all();
...
//------------------------------------------------------------------------
void Agg2D::polyline(agg::float64* xy, int numPoints)
{
    m_path.remove_all();
    //m_path.add_poly(xy, numPoints);
	m_path.concat_poly(xy,0,true); // JME
...
//------------------------------------------------------------------------
agg::float64 Agg2D::fontHeight() const
{
   return m_fontHeight;
}
...
//------------------------------------------------------------------------
agg::float64 Agg2D::textWidth(const char* str)
{
    agg::float64 x = 0;
    agg::float64 y = 0;
    bool first = true;
...
//------------------------------------------------------------------------
void Agg2D::text(agg::float64 x, agg::float64 y, const char* str, bool roundOff, agg::float64 ddx, agg::float64 ddy)
{
   agg::float64 dx = 0.0;
   agg::float64 dy = 0.0;
   ...
   agg::trans_affine  mtx;
    ...
    agg::float64 start_x = x + dx;
    agg::float64 start_y = y + dy;
...
//------------------------------------------------------------------------
void Agg2D::moveTo(agg::float64 x, agg::float64 y)
{
    m_path.move_to(x, y);
}

//------------------------------------------------------------------------
void Agg2D::moveRel(agg::float64 dx, agg::float64 dy)
{
...
//------------------------------------------------------------------------
void Agg2D::lineTo(agg::float64 x, agg::float64 y)
{
    m_path.line_to(x, y);
}
...
//------------------------------------------------------------------------
void Agg2D::lineRel(agg::float64 dx, agg::float64 dy)
{
...
//------------------------------------------------------------------------
void Agg2D::horLineTo(agg::float64 x)
{
    m_path.hline_to(x);
}
...
//------------------------------------------------------------------------
void Agg2D::horLineRel(agg::float64 dx)
{
    m_path.hline_rel(dx);
}
...
//------------------------------------------------------------------------
void Agg2D::verLineTo(agg::float64 y)
{
    m_path.vline_to(y);
}
...
//------------------------------------------------------------------------
void Agg2D::verLineRel(agg::float64 dy)
{
    m_path.vline_rel(dy);
}
...
//------------------------------------------------------------------------
void Agg2D::arcTo(agg::float64 rx, agg::float64 ry,
                  agg::float64 angle,
                  bool largeArcFlag,
                  bool sweepFlag,
...
//------------------------------------------------------------------------
void Agg2D::arcRel(agg::float64 rx, agg::float64 ry,
                   agg::float64 angle,
                   bool largeArcFlag,
                   bool sweepFlag,
...
//------------------------------------------------------------------------
void Agg2D::quadricCurveTo(agg::float64 xCtrl, agg::float64 yCtrl,
...
//------------------------------------------------------------------------
void Agg2D::quadricCurveRel(agg::float64 dxCtrl, agg::float64 dyCtrl,
...
//------------------------------------------------------------------------
void Agg2D::quadricCurveTo(agg::float64 xTo, agg::float64 yTo)
{
...
//------------------------------------------------------------------------
void Agg2D::quadricCurveRel(agg::float64 dxTo, agg::float64 dyTo)
{
...
//------------------------------------------------------------------------
void Agg2D::cubicCurveTo(agg::float64 xCtrl1, agg::float64 yCtrl1,
...
//------------------------------------------------------------------------
void Agg2D::cubicCurveRel(agg::float64 dxCtrl1, agg::float64 dyCtrl1,
...
//------------------------------------------------------------------------
void Agg2D::cubicCurveTo(agg::float64 xCtrl2, agg::float64 yCtrl2,
                         agg::float64 xTo,    agg::float64 yTo)
{
...
//------------------------------------------------------------------------
void Agg2D::cubicCurveRel(agg::float64 xCtrl2, agg::float64 yCtrl2,
                          agg::float64 xTo,    agg::float64 yTo)
{
...
//------------------------------------------------------------------------
void Agg2D::addEllipse(agg::float64 cx, agg::float64 cy, agg::float64 rx, agg::float64 ry, Direction dir)
{
                                             ...
                                             agg::float64 dstX1, agg::float64 dstY1, agg::float64 dstX2, agg::float64 dstY2)
{
    ...
    lineTo(dstX1, dstY2);
    closePolygon();
    agg::float64 parallelogram[6] = { dstX1, dstY1, dstX2, dstY1, dstX2, dstY2 };
    renderImage(img, imgX1, imgY1, imgX2, imgY2, parallelogram);
}

//------------------------------------------------------------------------
void Agg2D::transformImage(const Image& img, agg::float64 dstX1, agg::float64 dstY1, agg::float64 dstX2, agg::float64 dstY2)
{
    resetPath();
    moveTo(dstX1, dstY1);
    lineTo(dstX2, dstY1);
...
//------------------------------------------------------------------------
void Agg2D::transformImage(const Image& img, const agg::float64* parallelogram)
{
    resetPath();
    moveTo(parallelogram[0], parallelogram[1]);
    lineTo(parallelogram[2], parallelogram[3]);
...
//------------------------------------------------------------------------
void Agg2D::transformImagePath(const Image& img, agg::float64 dstX1, agg::float64 dstY1, agg::float64 dstX2, agg::float64 dstY2)
{
    agg::float64 parallelogram[6] = { dstX1, dstY1, dstX2, dstY1, dstX2, dstY2 };
    renderImage(img, 0, 0, img.renBuf.width(), img.renBuf.height(), parallelogram);
}
...
//------------------------------------------------------------------------
void Agg2D::transformImagePath(const Image& img, const agg::float64* parallelogram)
{
    renderImage(img, 0, 0, img.renBuf.width(), img.renBuf.height(), parallelogram);
}
...
void Agg2D::renderImage(const Image& img, int x1, int y1, int x2, int y2,
                        const agg::float64* parl)
{
    agg::trans_affine mtx((agg::float64)x1,
                          (agg::float64)y1,
                          (agg::float64)x2,
                          (agg::float64)y2,
                          parl);
...
struct Agg2DRasterizerGamma
{
    ...
    Agg2DRasterizerGamma(agg::float64 alpha, agg::float64 gamma) :
        m_alpha(alpha), m_gamma(gamma) {}

    agg::float64 operator() (agg::float64 x) const
    {
...
void Agg2D::blendImage(Image& img,
                       ...
                       agg::float64 dstX, agg::float64 dstY, unsigned alpha)
{
...
//------------------------------------------------------------------------
void Agg2D::blendImage(Image& img, agg::float64 dstX, agg::float64 dstY, unsigned alpha)
{
    worldToScreen(dstX, dstY);
    PixFormat pixF(img.renBuf);
...
//------------------------------------------------------------------------
void Agg2D::copyImage(Image& img, agg::float64 dstX, agg::float64 dstY)
{
    worldToScreen(dstX, dstY);
