...
namespace agg
{
    ...
    struct matrix_pivot
    {
        static int pivot(float64 m[Rows][Cols], unsigned row)
        {
            int k = int(row);
            float64 max_val, tmp;
    ...
    struct simul_eq
    {
        static bool solve(const float64 left[Size][Size], 
                          const float64 right[Size][RightCols],
                          float64 result[Size][RightCols])
        {
            unsigned i, j, k;
            float64 a1;

            float64 tmp[Size][Size + RightCols];
