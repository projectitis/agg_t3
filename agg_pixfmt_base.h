...
namespace agg
{
    ...
    struct blender_base
    {
        ...
        static rgba get(value_type r, value_type g, value_type b, value_type a, cover_type cover = cover_full)
        {
            ...
            {
                rgba c(
                    color_type::to_float64(r), 
                    color_type::to_float64(g), 
                    color_type::to_float64(b), 
                    color_type::to_float64(a));

                if (cover < cover_full)
                {
                    float64 x = float64(cover) / cover_full;
                    c.r *= x;
                    c.g *= x;
                    c.b *= x;
                    c.a *= x;
        ...
        {
            p[order_type::R] = color_type::from_float64(c.r);
            p[order_type::G] = color_type::from_float64(c.g);
            p[order_type::B] = color_type::from_float64(c.b);
            p[order_type::A] = color_type::from_float64(c.a);
        }
    };
}
